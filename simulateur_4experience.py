...
# ── Constantes physiques du tremplin réel (mesures terrain) ──
TREMPLIN_H     = 1.50    # Hauteur du tremplin réel (m)
ALPHA_EXIT_DEG = 45.0    # Angle de sortie du tremplin (°)
...
# ════════════════════════════════════════════════════════════════
# NOUVELLES PAGES PDF — format 16:9 paysage, charte burgundy
# Intégration avec pages du mémoire technique (memoire_4experience.pdf)
# ════════════════════════════════════════════════════════════════
...
PW16, PH16 = 13.333, 7.5   # 16:9 paysage (équiv. 960x540 pt)

def _logo_on_burgundy():
    """Logo blanc composité sur fond burgundy_dark."""
    try:
        ...
        a = arr[:, :, 3] / 255.0
        h = BUR_DARK.lstrip("#")
        bg = np.array([int(h[i:i+2], 16) for i in (0,2,4)], dtype=float)
        out = np.zeros((*arr.shape[:2], 3), dtype=np.uint8)
        for c in range(3):
            out[:,:,c] = (a*255 + (1-a)*bg[c]).clip(0,255)
        return out
    except Exception:
        return None

def _rect16(fig, l, b, w, h, color, zorder=0):
    fig.add_artist(mpatches.FancyBboxPatch(
        (l, b), w, h, boxstyle="square,pad=0",
        transform=fig.transFigure, facecolor=color,
        edgecolor="none", zorder=zorder, clip_on=False))

def _page_header(fig, titre):
    """En-tête burgundy commun (sauf couvertures)."""
    _rect16(fig, 0, 0.90, 1.0, 0.10, BUR_DARK)
    lw = 0.10; lh = lw * PW16 / (4.0 * PH16)
    lb = 0.90 + (0.10 - lh) / 2.0
    logo = _logo_on_burgundy()
    if logo is not None:
        ax_l = fig.add_axes([0.04, lb, lw, lh])
        ax_l.axis("off"); ax_l.set_xticks([]); ax_l.set_yticks([])
        ax_l.imshow(logo, aspect="auto", interpolation="lanczos")
    ax_t = fig.add_axes([0.55, 0.90, 0.42, 0.10])
    ax_t.set_facecolor(BUR_DARK); ax_t.axis("off"); ax_t.patch.set_visible(True)
    ax_t.text(0.95, 0.5, titre, fontsize=20, fontweight="bold",
              color="white", ha="right", va="center", transform=ax_t.transAxes)

def _page_footer(fig, num):
    _rect16(fig, 0, 0, 1.0, 0.03, BUR_DARK)
    ax_f = fig.add_axes([0, 0.035, 1, 0.025]); ax_f.axis("off")
    ax_f.text(0.04, 0.5, "4 Experience  |  Rapport technique",
              fontsize=8, color=BUR_GRIS, va="center")
    ax_f.text(0.96, 0.5, f"Page {num}", fontsize=8,
              color=BUR_GRIS, va="center", ha="right")

def _pdf_v2_cover():
    """Page 1 — couverture générique (sans nom de site)."""
    fig = plt.figure(figsize=(PW16, PH16)); fig.patch.set_facecolor("white")
    _rect16(fig, 0, 0, 0.30, 1.0, BUR_DARK)
    lw = 0.16; lh = lw * PW16 / (4.0 * PH16)
    logo = _logo_on_burgundy()
    if logo is not None:
        ax_l = fig.add_axes([0.07, 0.86, lw, lh])
        ax_l.axis("off"); ax_l.set_xticks([]); ax_l.set_yticks([])
        ax_l.imshow(logo, aspect="auto", interpolation="lanczos")
    ax_v = fig.add_axes([0, 0, 0.30, 1]); ax_v.axis("off")
    ax_v.add_patch(mpatches.Circle((0.5, 0.68), 0.04,
                                     transform=ax_v.transAxes,
                                     facecolor="none", edgecolor="white",
                                     linewidth=1.8))
    ax_v.text(0.5, 0.30, "Tubing", fontsize=58, fontweight="bold",
              color="white", ha="center", va="center",
              rotation=90, transform=ax_v.transAxes)
    # Panneau droit illustration
    ax_r = fig.add_axes([0.30, 0, 0.70, 1]); ax_r.axis("off")
    ax_r.add_patch(plt.Rectangle((0,0), 1, 1, facecolor="#B5D0E0",
                                   edgecolor="none", transform=ax_r.transAxes))
    mt_bg = plt.Polygon([(0,0),(0.25,0.55),(0.50,0.30),(0.75,0.65),
                          (1.0,0.50),(1.0,0)], facecolor="#C8DCE8",
                          edgecolor="none", transform=ax_r.transAxes)
    mt_fg = plt.Polygon([(0,0),(0.15,0.40),(0.40,0.20),(0.65,0.45),
                          (0.85,0.25),(1.0,0.35),(1.0,0)],
                          facecolor="#A8C2D3", edgecolor="none",
                          transform=ax_r.transAxes)
    ax_r.add_patch(mt_bg); ax_r.add_patch(mt_fg)
    ax_r.add_patch(plt.Rectangle((0,0), 1, 0.18, facecolor="white",
                                   edgecolor="none", transform=ax_r.transAxes))
    _rect16(fig, 0.36, 0.75, 0.60, 0.15, BUR_DARK)
    ax_t = fig.add_axes([0.36, 0.75, 0.60, 0.15])
    ax_t.set_facecolor(BUR_DARK); ax_t.axis("off"); ax_t.patch.set_visible(True)
    ax_t.text(0.5, 0.62, "Rapport technique",
              fontsize=30, fontweight="bold", color="white",
              ha="center", va="center", transform=ax_t.transAxes)
    ax_t.text(0.5, 0.25, "Simulation piste & trajectoire airbag",
              fontsize=14, color="#F4DCE3",
              ha="center", va="center", transform=ax_t.transAxes)
    return fig

def _pdf_v2_sommaire():
    """Page 7 — sommaire."""
    fig = plt.figure(figsize=(PW16, PH16)); fig.patch.set_facecolor("white")
    _page_header(fig, "Sommaire")
    ax = fig.add_axes([0.10, 0.10, 0.80, 0.75]); ax.axis("off")
    sections = [
        ("1.  PRÉSENTATION 4 EXPERIENCE",   "pp. 2 – 6",   BUR_DARK),
        ("2.  ÉTUDE DE VOTRE PROJET",       "pp. 8 – 14",  BUR_MED),
        ("    Profil, vitesse, temps, trajectoire airbag, sections", "", BUR_GRIS),
        ("3.  ACCOMPAGNEMENT & BENCHMARK",  "pp. 15 – 19", BUR_DARK),
        ("4.  NOS RÉFÉRENCES",              "pp. 20 – 45", BUR_DARK),
        ("5.  CLIENTS & SOLUTIONS",         "pp. 46 – 48", BUR_DARK),
    ]
    for k, (title, pages, color) in enumerate(sections):
        y = 0.92 - k * 0.13
        weight = "bold" if not title.startswith("    ") else "normal"
        size   = 14 if not title.startswith("    ") else 11
        ax.text(0.00, y, title, fontsize=size, fontweight=weight,
                color=color, transform=ax.transAxes)
        if pages:
            ax.text(1.00, y, pages, fontsize=12, color=BUR_MED,
                    ha="right", fontweight="bold", transform=ax.transAxes)
            ax.plot([0.45, 0.85], [y-0.015]*2, color="#cbd5e1",
                    linewidth=0.7, transform=ax.transAxes, linestyle=":")
    _page_footer(fig, 7)
    return fig

def _pdf_v2_project_cover(projet, site, reference, date_str, operateur, page_num=8):
    """Page 8 — couverture projet avec champs dynamiques."""
    fig = plt.figure(figsize=(PW16, PH16)); fig.patch.set_facecolor("white")
    _rect16(fig, 0, 0.82, 1.0, 0.18, BUR_DARK)
    _rect16(fig, 0, 0.812, 1.0, 0.008, BUR_MED)
    lw = 0.16; lh = lw * PW16 / (4.0 * PH16)
    lb = 0.82 + (0.18 - lh) / 2.0
    logo = _logo_on_burgundy()
    if logo is not None:
        ...
        ax_l.axis("off"); ax_l.set_xticks([]); ax_l.set_yticks([])
        ax_l.imshow(logo, aspect="auto", interpolation="lanczos")
    ax_t = fig.add_axes([0.30, 0.82, 0.66, 0.18])
    ax_t.set_facecolor(BUR_DARK); ax_t.axis("off"); ax_t.patch.set_visible(True)
    ax_t.text(0.97, 0.62, "Étude technique de votre projet",
              fontsize=22, fontweight="bold", color="white",
              va="center", ha="right", transform=ax_t.transAxes)
    ax_t.text(0.97, 0.25, "Simulation piste & trajectoire airbag",
              fontsize=12, color="#F4DCE3",
              va="center", ha="right", transform=ax_t.transAxes)
    ax_i = fig.add_axes([0.10, 0.22, 0.80, 0.55]); ax_i.axis("off")
    infos = [
        ("Projet",      projet or "—"),
        ("Site / Lieu", site or "—"),
        ("Référence",   reference or "—"),
        ("Date",        date_str or "—"),
        ("Opérateur",   operateur or "—"),
    ]
    for k, (lbl, val) in enumerate(infos):
        y = 0.92 - k * 0.16
        ax_i.text(0.00, y, lbl + " :", fontsize=14, fontweight="bold",
                  color=BUR_MED, va="center", transform=ax_i.transAxes)
        ax_i.text(0.25, y, str(val), fontsize=14,
                  color="#1e293b", va="center", transform=ax_i.transAxes)
        ax_i.plot([0, 1], [y-0.06]*2, color="#e2e8f0",
                  linewidth=0.6, transform=ax_i.transAxes)
    _rect16(fig, 0, 0, 1.0, 0.03, BUR_DARK)
    ax_f = fig.add_axes([0, 0.035, 1, 0.025]); ax_f.axis("off")
    ax_f.text(0.04, 0.5, "4 Experience  |  Rapport technique",
              fontsize=8, color=BUR_GRIS, va="center")
    ax_f.text(0.96, 0.5, f"Page {page_num}", fontsize=8,
              color=BUR_GRIS, va="center", ha="right")
    return fig

def _pdf_v2_chart_page(chart_fig, titre, page_num):
    """Page 9-13 — graphique du logiciel dans cadre burgundy."""
    fig = plt.figure(figsize=(PW16, PH16)); fig.patch.set_facecolor("white")
    _page_header(fig, titre)
    if chart_fig is not None:
        ax_box = fig.add_axes([0.08, 0.15, 0.84, 0.68])
        ax_box.set_xlim(0,1); ax_box.set_ylim(0,1); ax_box.axis("off")
        box = mpatches.FancyBboxPatch(
            (0.005, 0.005), 0.99, 0.99,
            boxstyle="round,pad=0,rounding_size=0.02",
            facecolor="white", edgecolor=BUR_MED, linewidth=1.5,
            transform=ax_box.transAxes)
        ax_box.add_patch(box)
        buf = io.BytesIO()
        chart_fig.savefig(buf, format="png", dpi=180,
                           bbox_inches="tight", facecolor="white", pad_inches=0.15)
        buf.seek(0)
        img = np.array(Image.open(buf))
        ax_im = fig.add_axes([0.10, 0.18, 0.80, 0.62])
        ax_im.axis("off")
        ax_im.imshow(img, aspect="auto", interpolation="lanczos")
    _page_footer(fig, page_num)
    return fig

def _pdf_v2_sections_no_mu(sections, page_num=14):
    """Page 14 — tableau des sections SANS colonne μ."""
    fig = plt.figure(figsize=(PW16, PH16)); fig.patch.set_facecolor("white")
    _page_header(fig, "Sections de piste")
    ax = fig.add_axes([0.08, 0.10, 0.84, 0.75]); ax.axis("off")
    ax.text(0, 1.0, "Profil détaillé de la piste", fontsize=12,
            fontweight="bold", color=BUR_MED, transform=ax.transAxes)
    ax.text(0, 0.96, f"{len(sections)} sections de la zone de départ jusqu'à la zone d'arrêt.",
            fontsize=10, color=BUR_GRIS, transform=ax.transAxes)
    rows = []
    for i, s in enumerate(sections):
        surface = s.get("cat", "—").replace("❄️ ","").replace("🟩 ","")\
                  .replace("🛝 ","").replace("⛷️ ","").strip()
        cond = "Sec" if s.get("cond") == "sec" else "Fart"
        rows.append([str(i+1), s.get("nom","—"),
                     f"{s.get('angle',0):.1f}°",
                     f"{s.get('longueur',0):.1f} m",
                     surface, cond])
    tbl = ax.table(cellText=rows,
                   colLabels=["#", "Nom", "Angle", "Longueur", "Surface", "État"],
                   cellLoc="center", loc="upper left",
                   bbox=[0, 0.0, 1, 0.85],
                   colWidths=[0.05, 0.28, 0.10, 0.13, 0.27, 0.17])
    tbl.auto_set_font_size(False); tbl.set_fontsize(11); tbl.scale(1, 1.5)
    for (r, c), cell in tbl.get_celld().items():
        cell.set_edgecolor("#e2e8f0")
        if r == 0:
            cell.set_facecolor(BUR_MED)
            cell.set_text_props(fontweight="bold", color="white")
        else:
            cell.set_facecolor("white" if r % 2 else BUR_PALE)
    _page_footer(fig, page_num)
    return fig

def _find_memoire_pdf():
    """Cherche le fichier mémoire dans le même dossier que le script ou en parent."""
    candidates = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "memoire_4experience.pdf"),
        os.path.join(os.getcwd(), "memoire_4experience.pdf"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "memoire_4experience.pdf"),
    ]
    for p in candidates:
        if os.path.exists(p):
...
with tab1:
    ...
    # Bouton simulation
    # Vérifier que toutes les sections sont remplies (angle = 0 autorisé pour zone d'arrêt)
    sec_ok = all(
        s.get("cat") and s.get("cond")
        and s.get("longueur", 0) > 0
...
with tab4:
    ...
    def generer_pdf(titre, projet, site, date_str, operateur, notes,
        ...
        """Génère le rapport selon la nouvelle structure (48 pages — fusion avec mémoire technique)."""

        # 1. Générer les pages custom dans un PDF temporaire (16:9 paysage)
        custom_buf = io.BytesIO()
        with PdfPages(custom_buf) as pdf:
            # Page 1 : couverture générique (sans nom de projet)
            f = _pdf_v2_cover(); pdf.savefig(f, bbox_inches="tight"); plt.close(f)
            # Page 7 : sommaire
            f = _pdf_v2_sommaire(); pdf.savefig(f, bbox_inches="tight"); plt.close(f)
            # Page 8 : couverture projet (champs dynamiques de l'app)
            f = _pdf_v2_project_cover(projet, site, titre, date_str, operateur, 8)
            pdf.savefig(f, bbox_inches="tight"); plt.close(f)
            # Pages 9-13 : courbes du simulateur (vraies fonctions du logiciel)
            ...
            page_num = [9]
            def _chart(chart_fig, titre_pg):
                pg = _pdf_v2_chart_page(chart_fig, titre_pg, page_num[0])
                pdf.savefig(pg, bbox_inches="tight"); plt.close(pg)
                page_num[0] += 1
            ...
            if piste_res:
                ...
                if incl_profil:
                    ...
                    _chart(fig, "Profil de piste + interpolation"); plt.close(fig)
                if incl_vitesse:
                    fig = fig_vitesse(piste_res)
                    _chart(fig, "Vitesse en fonction de la distance"); plt.close(fig)
                if incl_temps:
                    fig = fig_temps(piste_res)
                    _chart(fig, "Temps écoulé / distance"); plt.close(fig)
            if incl_airbag and ab_res:
                fig = fig_airbag(ab_res, ab_alpha, ab_hair)
                _chart(fig, "Trajectoire airbag"); plt.close(fig)
            if incl_cmp:
                ...
                if cmp_proj and cmp_terr:
                    ...
                    _chart(fig, "Profil comparatif"); plt.close(fig)
            # Page 14 : tableau sections (sans μ)
            if piste_secs:
                f = _pdf_v2_sections_no_mu(piste_secs, page_num[0])
                pdf.savefig(f, bbox_inches="tight"); plt.close(f)

        # 2. Fusionner avec le mémoire technique
        if not HAS_PYPDF:
            st.error(
                "⚠️ **La bibliothèque `pypdf` n'est pas installée.**\n\n"
                "Le rapport généré ne contient que les pages dynamiques (sans présentation 4 Experience ni références).\n\n"
                "**Solution** : ferme l'application, relance `1_INSTALLER.bat`, puis relance `2_LANCER.bat`."
            )
            return custom_buf.getvalue()

        memoire_path = _find_memoire_pdf()
        if not memoire_path:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            st.error(
                "⚠️ **Le fichier `memoire_4experience.pdf` est introuvable.**\n\n"
                "Sans ce fichier, le rapport ne contient que les pages dynamiques "
                "(sans présentation 4 Experience ni références aux projets passés).\n\n"
                f"**Emplacement attendu :** `{script_dir}\\memoire_4experience.pdf`\n\n"
                "**Solution** : décompresse à nouveau le fichier `4Experience_Simulateur_v3.zip` "
                "en écrasant les fichiers existants, puis relance le programme."
            )
            return custom_buf.getvalue()

        try:
            custom_buf.seek(0)
            custom_reader = pypdf.PdfReader(custom_buf)
            memoire = pypdf.PdfReader(memoire_path)
            writer = pypdf.PdfWriter()

            # Ordre final :
            # Page 1     : custom[0] (couverture générique)
            # Pages 2-6  : memoire pages 2-6 (indices 1-5) — intro générique
            # Page 7     : custom[1] (sommaire)
            # Pages 8-N  : custom[2..fin] (projet : cover + courbes + sections)
            # Pages N+1+ : memoire pages 18-51 (indices 17-50) — accompagnement, références, etc.

            writer.add_page(custom_reader.pages[0])  # cover générique
            for i in range(1, 6):
                writer.add_page(memoire.pages[i])    # mémoire pages 2-6
            for i in range(1, len(custom_reader.pages)):  # sommaire + projet
                writer.add_page(custom_reader.pages[i])
            for i in range(17, 51):
                writer.add_page(memoire.pages[i])    # mémoire pages 18-51

            d = writer.add_metadata({
                "/Title": titre or "Rapport 4 Experience",
                "/Author": "4 Experience Simulateur",
                "/Subject": "Simulation de piste & trajectoire airbag",
            })
            ...
            st.error(f"Erreur lors de la fusion avec le mémoire technique : {e}")
            return custom_buf.getvalue()
